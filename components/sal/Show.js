...
export default function Show({ sal }) {
	...
	const clear = () => {
		...
		setSearchTerm('');
		setSearchResults(
			sal.sort((a, b) => (a.salary_date < b.salary_date ? 1 : -1))
		);

		setMonthNumber(1);

		//setSearchResults(sal.sal);
		setCarLength(sal.length);
		inputReference.current.focus();
	};

	const searchByTime = (time) => {
		const results = sal.filter((s) =>
	...
	function printPdf() {
		...
		return toPDF(rows, columns, `${rows[0].name} :تفاصيل رواتب الموظف`);
	}

	return (
		<>
			<Container>
				<Title title={`تفاصيل رواتب الموظف: ${sal[0].emp_name} `} />
