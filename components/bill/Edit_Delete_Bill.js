...
import ConfirmationModal, { Spans, Title } from '../comUtil/ComUtil';
...
export default function Edit_Delete_Bill({ formData }) {
		...
		if (show)
			return (
				<ConfirmationModal
					...
					handleDelete={async () =>
						handleDelete('showBillPage', 'bills', formData._id, router)
					}
					car={formData.bill_number}
				/>
			);
		return;
	}
	...
	async function handelArchive(values) {
		if (confirm(`هل تريد نقل  الفاتورة الحالية إلى الأرشيف`)) {
	...
	async function put(values) {
		...
		values.check_date = moment(values.check_date).format('YYYY-MM-DD');
		values.bill_amount = toCurrency(values.bill_amount);
		handlePut(values, values.bill_number, 'bills', router);
